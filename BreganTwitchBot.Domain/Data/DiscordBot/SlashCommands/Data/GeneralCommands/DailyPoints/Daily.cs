...
namespace BreganTwitchBot.Domain.Data.DiscordBot.SlashCommands.Data.GeneralCommands.DailyPoints
{
    public class DailyData
    {
        public static async Task<EmbedBuilder> HandleDailyCommand(SocketInteractionContext context)
        {
            ...
            {
                user = dbContext.Users.Include(x => x.DailyPoints).Where(x => x.DiscordUserId == context.User.Id).First();
            }
            ...
            using (var dbContext = new DatabaseContext())
            {
                var userToUpdate = dbContext.Users.Include(x => x.DailyPoints).Where(x => x.DiscordUserId == context.User.Id).First();
                userToUpdate.DailyPoints.DiscordDailyStreak++;
                userToUpdate.DailyPoints.DiscordDailyTotalClaims++;
                userToUpdate.DailyPoints.DiscordDailyClaimed = true;
                dbContext.SaveChanges();
            }
