...
using BreganTwitchBot.Domain.Data.DiscordBot.SlashCommands.Data.Gambling.Enums;
...
namespace BreganTwitchBot.Domain.Data.DiscordBot.SlashCommands.Data.Gambling
{
    public class GamblingData
    {
        public static async Task<EmbedBuilder> HandleSpinCommand(string pointsBeingGambled, SocketInteractionContext context)
        {
            ...
            if (pointsBeingGambled.ToLower() == "all")
            {
                ...
                embed.WithAuthor($"Spin - {context.User.Username} - {user.Points:N0} {AppConfig.PointsName} Gambled", null, context.User.GetAvatarUrl());
            ...
            embed.WithAuthor($"Spin - {context.User.Username} - {convertedPoints:N0} {AppConfig.PointsName} Gambled", null, context.User.GetAvatarUrl());
        ...
        private static async Task<EmbedBuilder> SpinPoints(long points, ulong discordId, EmbedBuilder embed, ulong channelId)
        {
             ...
             */
            await DiscordHelper.RemoveUserPoints(discordId, points);
            ...
            switch (randomSpin)
            {
                case 73:
                    ...
                    embed.AddField(AppConfig.PointsName, $"{userPoints:N0}");
                    BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 500, channelId));
                    break;
                ...
                case <= 20:
                    if (random.Next(1, 4) == 1)
                    {
                        ...
                        await DiscordHelper.AddUserPoints(discordId, points * 350);
                        AddWin(WinTypes.EggplantWins);

                        embed.AddField(AppConfig.PointsName, $"{points * 350 + userPoints:N0}");
                        BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 100, channelId));
                    }
                    else
                    {
                        embed.Description = @$"**─────────**
                        ...
                        await DiscordHelper.AddUserPoints(discordId, 1);
                        AddWin(WinTypes.CheeseWins);

                        embed.AddField(AppConfig.PointsName, $"{userPoints + 1:N0}");
                        BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 1, channelId));
                    }
                    break;
                ...
                case <= 50:
                    ...
                    await DiscordHelper.AddUserPoints(discordId, points * 80);
                    AddWin(WinTypes.CucumberWins);

                    embed.AddField(AppConfig.PointsName, $"{points * 80 + userPoints:N0}");
                    BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 50, channelId));
                    break;
                ...
                case <= 150:
                    if (random.Next(1, 3) == 1)
                    {
                        ...
                        await DiscordHelper.AddUserPoints(discordId, points * randomPayout);
                        AddWin(WinTypes.CherriesWins);

                        embed.AddField(AppConfig.PointsName, $"{points * randomPayout + userPoints:N0} Remaining");
                        BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 40, channelId));
                    }
                    else
                    {
                        embed.Description = @$"**─────────**
                        ...
                        await DiscordHelper.AddUserPoints(discordId, points * 30);
                        AddWin(WinTypes.PineappleWins);

                        embed.AddField(AppConfig.PointsName, $"{points * 30 + userPoints:N0}");
                        BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 40, channelId));
                    }
                    break;
                ...
                case <= 550:
                    ...
                    await DiscordHelper.AddUserPoints(discordId, points * 15);
                    AddWin(WinTypes.GrapesWins);

                    embed.AddField(AppConfig.PointsName, $"{points * 15 + userPoints:N0}");
                    BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 30, channelId));
                    break;
                ...
                default:
                    if (random.Next(0, 1000000) == 7373)
                    {
                        ...
                        BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 10000, channelId));
                        break;
                    }
                    ...
                    embed.AddField(AppConfig.PointsName, $"{userPoints:N0}");
                    BackgroundJob.Enqueue(() => DiscordLevelling.AddDiscordXp(discordId, 5, channelId));
                    break;
            }
