...
namespace BreganTwitchBot.Core.Twitch.Commands.Modules.WordBlacklist
{
    public class WordBlacklist
    {
        ...
        public static void OnMessageReceived(OnMessageReceivedArgs e)
        {
            //for the /me melvins
            ...
            if (e.ChatMessage.IsModerator || e.ChatMessage.IsBroadcaster || SuperMods.SuperMods.IsUserSupermod(e.ChatMessage.UserId))
            {
                return;
            }
            ...
            if (predictionResult.Prediction == 1 || _strikeWords.Any(rankBeggarRegex.Contains))
            {
                ...
                if (totalMessages < 10)
                {
                    ...
                    TwitchHelper.BanUser(e.ChatMessage.Username, "Joining and begging");
                }
                ...
                if (totalMessages < 20)
                {
                    if (_strikedUsers.Contains(e.ChatMessage.Username))
                    {
                        ...
                        TwitchHelper.BanUser(e.ChatMessage.Username, "continued to beg after being warned for begging at low message count");
                        return;
                    }
            ...
            {
                TwitchHelper.BanUser(e.ChatMessage.Username, "blocksWOT automated rule - very offensive word");
        ...
        private static void AddBlacklistedItem(string word, string type)
        {
            using (var context = new DatabaseContext())
            {
                context.Blacklist.Add(new Data.Models.Blacklist
                {
                    WordType = type,
                    Word = word
                });
        ...
        private static void RemoveBlacklistedItem(string word, string type)
        {
            using (var context = new DatabaseContext())
            {
                var wordToRemove = new Data.Models.Blacklist
                {
                    Word = word,
                    WordType = type
                };
