...
namespace BreganTwitchBot.Domain.Data.DiscordBot.SlashCommands.Data.HoursPoints
{
    public class HoursPointsData
    {
        public static async Task<EmbedBuilder> HandleHoursCommand(SocketInteractionContext ctx, string optUsername, IUser discordUser)
        {
            ...
            if (optUsername != null)
            {
                ...
                {
                    var user = context.Users.Include(x => x.Watchtime).Where(x => x.Username == optUsername).FirstOrDefault();

                    if (user != null)
                    {
                        twitchUserMinutes = TimeSpan.FromMinutes(user.Watchtime.MinutesInStream);
                    }
                }
            ...
            if (discordUser == null)
            {
                using (var context = new DatabaseContext())
                {
                    var user = context.Users.Include(x => x.Watchtime).Where(x => x.DiscordUserId == ctx.User.Id).FirstOrDefault();

                    if (user != null)
                    {
                        discordUserMinutes = TimeSpan.FromMinutes(user.Watchtime.MinutesInStream);
                    }
                }
            }
            else
            {
                using (var context = new DatabaseContext())
                {
                    var user = context.Users.Include(x => x.Watchtime).Where(x => x.DiscordUserId == discordUser.Id).FirstOrDefault();
        ...
        public static async Task<string> HandlePrestigeCommand(SocketInteractionContext ctx)
        {
            ...
            using (var context = new DatabaseContext())
            {
                var user = context.Users.Include(x => x.DiscordUserStats).Where(x => x.DiscordUserId == ctx.User.Id).First();

                user.DiscordUserStats.PrestigeLevel++;
                user.Points -= AppConfig.PrestigeCap;
