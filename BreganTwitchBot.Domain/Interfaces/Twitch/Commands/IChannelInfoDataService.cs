...
namespace BreganTwitchBot.Domain.Interfaces.Twitch.Commands
{
    public interface IChannelInfoDataService
    {
        ...
        Task<string> GetTitleAsync(ChannelChatMessageReceivedParams msgParams);

        /// <summary>
        /// Sets the channel's stream title. Moderator only.
        /// </summary>
        /// <returns>The message to send back to chat</returns>
        Task<string> SetTitleAsync(ChannelChatMessageReceivedParams msgParams, string newTitle);

        /// <summary>
        /// Gets the channel's current game
        /// </summary>
        /// <returns>The message to send back to chat</returns>
        Task<string> GetGameAsync(ChannelChatMessageReceivedParams msgParams);

        /// <summary>
        /// Sets the channel's game. Moderator only.
        /// </summary>
        /// <returns>The message to send back to chat</returns>
        Task<string> SetGameAsync(ChannelChatMessageReceivedParams msgParams, string newGame);
    }
}
