...
namespace BreganTwitchBot.Core.DiscordBot.Commands.Modules.GeneralCommands.BlocksSocks
{
    public class BlocksSocks
    {
        public static async Task<EmbedBuilder> GetSkinAndAddSocks(string playerName, ulong discordId)
        {
            ...
            var channel = DiscordConnection.DiscordClient.GetGuild(Config.DiscordGuildID).GetChannel(713365310408884236) as IMessageChannel; //blockssocks channel
            try
            {
                //Create the request
