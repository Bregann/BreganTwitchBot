        ...
        public async Task<string> HandleLeaderboardCommand(ChannelChatMessageReceivedParams msgParams, LeaderboardType type)
        {
            ...
            switch (type)
            {
                case LeaderboardType.Points:
                    top5 = context.ChannelUserData.Where(x => x.Channel.BroadcasterTwitchChannelId == msgParams.BroadcasterChannelId)
                        .OrderByDescending(x => x.Points)
                        .Include(x => x.ChannelUser)
                        .Take(5)
                        .ToDictionary(x => x.ChannelUser.TwitchUsername, x => x.Points);
                    break;
