...
namespace BreganTwitchBot.Domain.Bot.Twitch.Commands.Modules.Points
{
    public class Points
    {
        public static void HandlePointsCommand(OnChatCommandReceivedArgs command)
        {
            ...
            {
                TwitchHelper.SendMessage($"@{command.Command.ChatMessage.Username} => You have {PointsHelper.GetUserPoints(command.Command.ChatMessage.Username):N0} {AppConfig.PointsName}. Rank: {GetPointsRank(command.Command.ChatMessage.Username)}");
                return;
            }

            TwitchHelper.SendMessage($"@{command.Command.ChatMessage.Username} => {command.Command.ArgumentsAsList[0].Replace("@", "")} has {PointsHelper.GetUserPoints(command.Command.ArgumentsAsList[0].ToLower().Replace("@", "")):N0} {AppConfig.PointsName} Rank: {GetPointsRank(command.Command.ArgumentsAsList[0].ToLower())}");
        }
        ...
        public static void HandleAddPointsCommand(OnChatCommandReceivedArgs command)
        {
            ...
            {
                TwitchHelper.SendMessage($"@{command.Command.ChatMessage.Username} => The format for adding {AppConfig.PointsName} is !addpoints <username> <points>");
